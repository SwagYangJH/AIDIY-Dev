...
const ForgotPassword = () => {
  ...
  const [step, setStep]       = useState(1);          // 1: email, 2: new pwd

  /* ─────────────────  jump to step-2 after OTP verified ─────────────── */
  useEffect(() => {
    if (location.state?.allowReset) {                 // ← NEW
  ...
  const handleSendOTP = async e => {
    ...
    try {
      ...
      } else setErrors({ submit: data.error || 'Failed to send OTP' });
  ...
  const handleResetPassword = async e => {
    ...
    setLoading(true);
    try {
      const r = await fetch('http://localhost:5500/api/auth/reset-password', {
        method:'POST', headers:{'Content-Type':'application/json'},
      ...
      } else setErrors({ submit: data.error || 'Failed to reset password' });
    } catch { setErrors({ submit:'Network error. Try again.' }); }
    finally { setLoading(false); }
  };
              ...
              <form onSubmit={step===1 ? handleSendOTP : handleResetPassword}>
                <div className={tw('space-y-4')}>
                  ...
                  <div>
                    ...
                    />
                    {errors.email && <p className={tw('text-red-500 text-xs mt-1')}>{errors.email}</p>}
                  </div>
                  ...
                  {step===2 && (
                    <>
                      {/* new password */}
                      <div>
                        <label className={tw('block text-sm font-medium text-gray-700 mb-1')}>New password</label>
                        <input
                          type="password" name="newPassword" value={formData.newPassword}
                          onChange={handleChange} placeholder="Enter new password"
                          className={tw('w-full px-4 py-3 bg-gray-100 border border-gray-200 rounded-lg focus:outline-none focus:border-sky-400 transition-colors')}
                        />
                        {errors.newPassword && <p className={tw('text-red-500 text-xs mt-1')}>{errors.newPassword}</p>}
                      </div>
                      {/* confirm */}
                      <div>
                        <label className={tw('block text-sm font-medium text-gray-700 mb-1')}>Confirm password</label>
                        <input
                          type="password" name="confirmPassword" value={formData.confirmPassword}
                          onChange={handleChange} placeholder="Confirm password"
                          className={tw('w-full px-4 py-3 bg-gray-100 border border-gray-200 rounded-lg focus:outline-none focus:border-sky-400 transition-colors')}
                        />
                        {errors.confirmPassword && <p className={tw('text-red-500 text-xs mt-1')}>{errors.confirmPassword}</p>}
                      </div>
                    </>
                  )}
                </div>

                {errors.submit && <p className={tw('text-red-500 text-sm text-center mt-4')}>{errors.submit}</p>}
